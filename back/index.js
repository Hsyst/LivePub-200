...
app.get('/release-live', async (req, res) => {
  ...
  const m3u8Url = `https://hls.livepub.hsyst.xyz`;

  try {
    const response = await axios.head(`${m3u8Url}/${channel}.m3u8`);
    console.log(`${m3u8Url}/${channel}.m3u8 com status ${response.status}`)
    const responsagem = `${response.status}`
    if (responsagem !== "200") {
    ...
    db.run('INSERT INTO live_streams (channel_name, m3u8_url, createdIn, createdAt) VALUES (?, ?, ?, ?)',
      [channel, m3u8Url, createdIn, createdAt], function(err) {
        ...
        res.json({ message: `Canal ${channel} está liberado com sucesso e sua transmissão já está disponível para o público. Para encerrar a transmissão ao vivo, acesse: http://localhost:${PORT}/rem-live?channel=${channel}&key=${key}` });
    });
  } catch (error) {
